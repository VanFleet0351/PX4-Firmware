 ...
 *
 * Turns the automatic airspeed scale (scale from IAS to CAS/EAS) on or off. It is recommended level (kepping altitude) while performing the estimation. Set to 1 to start estimation (best when already flying). Set to 0 to end scale estimation. The estimated scale is then saved in the ARSP_ARSP_SCALE parameter.
 *
 * @boolean
 * @group Airspeed Validator
 */
